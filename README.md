...
Das Loginformular wird über `memberLogin.id` erkannt. Nach dessen `submitDone` aktualisiert das Portal die Person. `/webhook/me` erwartet eine Person und `gefunden: true` oder `erfolgreich: true`:
