...
| `gzfld02` | Originalwert der Berechnung aus MGVO. Wird im Serviceportal nicht mehr umgewandelt. |
| `beitrag` | Normalisierter langer Beitragswert für den Web-Trainingsplan. |
| `beitrag_kurz` | Normalisierter kurzer Beitragswert, insbesondere für den PDF-Trainingsplan. |
...
Die normalisierten Beitragsfelder werden vom vorgelagerten MGVO-Workflow geliefert:

- `beitrag` wird im Web-Trainingsplan angezeigt, zum Beispiel `Leistungsbeitrag`, `Basisbeitrag` oder `-`.
- `beitrag_kurz` ist für kompakte Ausgaben wie den PDF-Trainingsplan vorgesehen, zum Beispiel `L`, `B` oder `-`.
- `gzfld02` bleibt als originales MGVO-Feld erhalten und wird im Serviceportal nicht erneut umgewandelt.
...
| Zusatzfeld 2: Berechnung | `gzfld02` | Originalwert; Normalisierung erfolgt im vorgelagerten Workflow |
